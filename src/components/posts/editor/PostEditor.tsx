...
import { cn, removeAttrFromObject } from "@/lib/utils";
...
export default function PostEditor() {
  ...
  const rootProps = removeAttrFromObject(getRootProps(), "onClick");
  ...
  function onSubmit() {
    mutation.mutate(
      {
        content: input,
        mediaIds: attachments.map((attachment) => attachment.mediaId).filter(Boolean) as string[],
      },
      {
        onSuccess() {
    ...
    <div className="flex flex-col gap-5 rounded-2xl bg-card p-5 shadow-xl">
      ...
      <div className="flex items-center justify-end gap-3">
        ...
        <AddAttachmentsButton
          ...
          disabled={isUploading || attachments.length >= MAX_NUM_FILES_PER_UPLOAD}
        />
        <LoadingButton
          onClick={onSubmit}
...
function AttachmentPreview({
  ...
  return (
    <div className={cn("relative mx-auto size-fit", isUploading && "opacity-50")}>
      ...
      {/* UploadThing limitation: Cannot cancel active uploads so don't allow attachment removal if it is currently uploading */}
      {!isUploading && (
        <button
