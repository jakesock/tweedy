...
import { getCommentsQueryKey } from "@/lib/utils";
...
export default function Comments({ post }: CommentsProps) {
  ...
  const queryKey = getCommentsQueryKey(post.id);
    ...
    <div className="space-y-3">
      ...
      {status === "success" && !comments.length && (
        <p className="text-center text-muted-foreground">
          There&apos;s nothing here, be the first to reply!
        </p>
      )}
      {status === "error" && (
        <p className="text-center text-destructive">An error occured while loading replies.</p>
      )}
